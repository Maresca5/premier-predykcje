...
if not historical.empty:
    ...
    with tab3:
        ...
        with st.expander("💰 Symulacja Kelly – Equity Curve (Walk-Forward)", expanded=False):
            # TRUE WALK-FORWARD: dla meczu z kolejki K, model trenuje na danych K-1
            # Eliminuje look-ahead bias
            ...
            if not _hist_curr_eq.empty and not _sched_played.empty and len(_hist_curr_eq) >= 5:
                ...
                _rundy = sorted(_hist_eq_sorted["_round"].unique())
                _csv_code_eq = LIGI[wybrana_liga]["csv_code"]
                _eq_rows = []
                        ...
                        for _, _hr in _hist_eq_sorted[_hist_eq_sorted["_round"] == _rnd].iterrows():
                            ...
                            try:
                                ...
                                _pred_kf = predykcja_meczu(
                                    ...
                                    csv_code=_csv_code_eq, n_train=_n_kf)
                            except Exception:
                                continue

                            _typ_kf = _pred_kf["typ"]
                            _p_kf   = _pred_kf["p_typ"]
                            _fo_kf  = _pred_kf["fo_typ"]

                            try:
                                _hg = int(_hr["FTHG"]); _ag = int(_hr["FTAG"])
                            except Exception:
                                continue
                            ...
                            _k_kf = (_gk("MaxCH","MaxCD","MaxCA",_hr,_typ_kf) or
                                     _gk("PSCH","PSCD","PSCA",_hr,_typ_kf) or
                                     _gk("PSH","PSD","PSA",_hr,_typ_kf) or
                                     _gk("B365H","B365D","B365A",_hr,_typ_kf) or
                                     _fo_kf)
            ...
            if len(_eq_df) >= 3:
                ...
                _src = [f"Kursy: **{_n_csv_ok}** z CSV (MaxC/Pinnacle/B365)"]
                if _n_fair_fallback:
                    _src.append(f"**{_n_fair_fallback}** fair odds (brak kursu)")
                st.caption("  .  ".join(_src))

                _bk = _KS; _bk_flat = _KS
                _bk_vals = []; _flat_vals = []
                ...
                for _kol, _grp in _eq_df.groupby("kolejnosc"):
                    for _, _r in _grp.iterrows():
                        _kf_real = float(_r["kurs_buk"]) if (
                            _r["kurs_buk"] is not None and not pd.isna(_r["kurs_buk"])
                        ) else float(_r["fair_odds"])
                        _pv = _kf_real - 1 if _r["trafione"] == 1 else -1
                        _bk_flat += _pv
                        _flat_vals.append(round(_bk_flat, 2))

                    _cand = _grp[_grp["ev"].notna() & (_grp["ev"] >= _KEV)].nlargest(3, "ev")
                    for _, _r in _cand.iterrows():
                        _pt  = _conservative_p(_r["p_model"])
                ...
                _kelly_hr = f"{_kelly_traf/_kelly_typy:.0%}" if _kelly_typy else "brak"

                st.markdown(
                    "<div class='section-header'>Symulacja Kelly (walk-forward, bez look-ahead)"
                    "</div>", unsafe_allow_html=True)

                st.session_state[f"ml_bk_{wybrana_liga}"] = {
                    "bk": round(_bk,2), "bk_flat": round(_bk_flat,2),
                    "roi": round(_roi_kelly,2), "roi_flat": round(_roi_flat,2),
                    "n": _kelly_typy, "dd": round(_max_dd_k,2),
                    "bk_vals": _bk_vals, "kol_nums": list(_kol_nums),
                }

                _ek1, _ek2, _ek3, _ek4 = st.columns(4)
                _ek1.metric("Bankroll", f"{_bk:.0f} zl", f"{_roi_kelly:+.1f}%")
                _ek2.metric("Typow Kelly", _kelly_typy, f"Hit {_kelly_hr}", delta_color="off")
                _ek3.metric("Max Drawdown", f"{_max_dd_k:.1f}%",
                            "wysoki" if _max_dd_k > 20 else "ok",
                            delta_color="inverse" if _max_dd_k > 20 else "normal")
                _ek4.metric("Flat", f"{_bk_flat:.0f} zl", f"{_roi_flat:+.1f}%",
                            delta_color="normal" if _roi_flat >= 0 else "inverse")

                if len(_kol_nums) >= 2:
                    import altair as _alt
                    _cdf = pd.DataFrame({"Kolejka": _kol_nums[:len(_bk_vals)], "Kelly": _bk_vals})
                    _ln  = _alt.Chart(_cdf).mark_line(color=_kc, strokeWidth=2).encode(
                        x="Kolejka:Q", y=_alt.Y("Kelly:Q", title="Bankroll (zl)"))
                    _rl  = _alt.Chart(pd.DataFrame({"y":[_KS]})).mark_rule(
                        color="#555", strokeDash=[4,4]).encode(y="y:Q")
                    st.altair_chart((_ln+_rl).properties(height=180), use_container_width=True)
                ...
                if _kelly_typy > 0:
                    ...
                    st.caption(
                        f"{_kelly_typy} typow z EV>=5% . PnL: {_kelly_pnl:+.0f} zl . "
                        f"Kolejek bez kursow CSV: {_missing_kols}/{_kols_total}")

                    with st.expander(f"Szczegoly typow Kelly ({_kelly_typy})", expanded=False):
                        _kelly_rows = []
                        for _kol2, _grp2 in _eq_df.groupby("kolejnosc"):
                            _cand2 = _grp2[_grp2["ev"].notna() & (
                                _grp2["ev"] >= _KEV)].nlargest(3, "ev")
                            for _, _r2 in _cand2.iterrows():
                                ...
                                _ff = " f" if (_kb2 is None or pd.isna(_kb2)) else ""
                                _kelly_rows.append({
                                    "Kolejka": int(_kol2),
                                    ...
                                    "Kurs":    f"{_ke2:.2f}{_ff}",
                                    "EV":      f"{_ev2:+.1%}",
                                    "Stawka":  f"{_stw2:.0f} zl",
                                    "Wynik":   "TAK" if _traf2 else "NIE",
                                    "PnL":     f"{_pnl2:+.0f} zl",
                                })
                        if _kelly_rows:
                            st.dataframe(pd.DataFrame(_kelly_rows),
                                use_container_width=True, hide_index=True)
                            _wins = sum(1 for r in _kelly_rows if "TAK" in r["Wynik"])
                            st.caption(f"Trafione: {_wins} / {len(_kelly_rows)}"
                                       f" . f = fair odds (brak closing w CSV)")
            else:
                st.info("Za malo danych (minimum 3 mecze).")
