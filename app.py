"""
app.py – Streamlit UI. Logika w: config.py, data.py, model.py, db.py, render.py
"""
...
# ── Moduły projektu ──────────────────────────────────────────────────────────
from config import (
    LIGI, DB_FILE, BIEZACY_SEZON, SOT_BLEND_W,
    KALIBRACJA_PER_LIGA, SHRINK_ALPHA, PROG_PEWNY, PROG_PODWOJNA,
    MARKET_NOISE_MAX, KELLY_FRACTION, KELLY_BANKROLL_DEFAULT,
    KELLY_PROB_SCALE, KELLY_FRAC_SCALE, KELLY_FRACTIONS, MAX_EXPOSURE_PCT,
)
from data import (
    waga_poprzedniego, normalize_name, map_nazwa,
...
from model import (
    ...
    quality_score, entropy_kelly_multiplier, confidence_score, fair_odds,
    predykcja_meczu, alternatywne_zdarzenia,
)
from db import (
    init_db, zapisz_zdarzenia, aktualizuj_wynik_zdarzenia, aktualizuj_wyniki_z_api,
    statystyki_skutecznosci, kalibracja_modelu, metryki_globalne,
    model_sharpness_vs_rynek, rolling_stats, brier_per_kolejka,
    zapisz_wynik_bankroll, pobierz_bankroll_history, pobierz_aktualny_bankroll,
    zapisz_paper_trades, zapisz_real_odds, usun_paper_trade,
    pobierz_paper_trades, _ensure_paper_trades_table, rozlicz_paper_trades,
)
from render import (
    _forma_badge, _forma_html, _trend_forma, _stat_bar, render_forma_kontekst,
    koloruj, oblicz_p, badge_typ, render_macierz_html, macierz_goli_p,
    ostrzezenie_sedziego, deep_data_stats,
)
...
try:
    ...
    try:
        ...
        pass
    _br.start(tg_token=_br_tg_token, tg_chat=_br_tg_chat, fd_api_key=_br_fd_key)
except Exception:
    pass

# ── Pomocnicza funkcja kursów DC (tylko UI) ──────────────────────────────────
def _kurs_dc_live(typ, oh, od, oa):
    """Kurs DC i impl dla live odds."""
    try:
...
if not historical.empty:
    ...
    with tab1:
        ...
        # ── Briefing kolejki – Stability Score per liga ────────────────────────
        with st.expander("🛡️ Briefing kolejki – stabilność lig", expanded=False):
    ...
    with tab5:
        ...
        _lab_t1, _lab_t2, _lab_t3, _lab_t4 = st.tabs(["🧱 Bet Builder", "🔬 Symulacje Monte Carlo", "🤖 AutoML", "📋 Paper Betting"])
