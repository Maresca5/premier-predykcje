...
if not historical.empty:
    ...
    with tab3:
        ...
        if not power_df.empty:
            # ── SVG Power Rankings chart ──────────────────────────────────
            _dd_sorted = power_df.sort_values("_gol_str", ascending=False).head(20)
            _W, _H, _P = 620, max(220, len(_dd_sorted)*28+80), 170
            _max_g = _dd_sorted["_gol_str"].max() if not _dd_sorted.empty else 1
            _bars = []
            for _i, (_, _dr) in enumerate(_dd_sorted.iterrows()):
                _y   = _P//3 + _i*28
                _blen = _dr["_gol_str"] / _max_g * (_W - _P - 20)
                _forma = int(_dr.get("_forma", 0) or 0)
                _fc   = "#4CAF50" if _forma >= 9 else ("#FF9800" if _forma >= 6 else "#F44336")
                _sot  = float(_dr.get("SOT/M", 0) or 0)
                _kart = float(_dr.get("Kartki/M", 0) or 0)
                _rozn = float(_dr.get("Rożne/M", 0) or 0)
                _styl = str(_dr.get("Styl", "") or "")[:18]
                _bars.append(
                    f"<rect x='{_P}' y='{_y+5}' width='{_blen:.0f}' height='16' "
                    f"fill='#4CAF50' fill-opacity='0.7' rx='3'/>"
                    f"<text x='{_P-5}' y='{_y+16}' text-anchor='end' "
                    f"font-size='10' fill='#aaa' font-family='sans-serif'>{str(_dr['Drużyna'])[:16]}</text>"
                    f"<text x='{_P+_blen+5:.0f}' y='{_y+16}' "
                    f"font-size='9' fill='#888' font-family='sans-serif'>"
                    f"{_dr['Gole/M ↑']}g · {_sot:.1f}SOT · {_kart:.1f}YC · {_rozn:.1f}C</text>"
                    f"<circle cx='{_P-22}' cy='{_y+13}' r='5' fill='{_fc}'/>"
                )
            _svg = (
                f'<svg width="{_W}" height="{_H}" '
                f'style="background:var(--stat-bg);border-radius:8px;display:block;margin:auto">'
                f'<text x="{_W//2}" y="18" text-anchor="middle" '
                f'font-size="11" fill="#888" font-family="sans-serif">Gole/mecz (bieżący sezon)</text>'
                f'{"".join(_bars)}</svg>'
            )
            st.markdown(_svg, unsafe_allow_html=True)
            st.caption("● Forma (zielony=dobra, pomarańczowy=średnia, czerwony=słaba) · SOT=celne strzały · YC=kartki · C=rożne/M")

            # ── Tabela z kluczowymi metrykami ─────────────────────────────
            _show_cols = ["Drużyna", "M", "Gole/M ↑", "Strac./M ↓", "SOT/M",
                          "Kartki/M", "Rożne/M", "Styl", "Forma (pkt/5M)"]
            _avail = [c for c in _show_cols if c in power_df.columns]
            _display_df = power_df[_avail].sort_values("Gole/M ↑", ascending=False).reset_index(drop=True)
            st.dataframe(_display_df, use_container_width=True, hide_index=True,
                         column_config={
                             ...
                             "Kartki/M":        st.column_config.NumberColumn("YC/M", format="%.1f"),
                             "Rożne/M":         st.column_config.NumberColumn("C/M", format="%.1f"),
                             "Forma (pkt/5M)":  st.column_config.NumberColumn("Forma", format="%d"),
                         })
        else:
