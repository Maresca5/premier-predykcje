...
def init_db():
    ...
    con.execute("""
        CREATE TABLE IF NOT EXISTS odds_history (
            ...
            UNIQUE(liga, home, away, is_opening) ON CONFLICT IGNORE
        )
    """)
    con.commit()
    con.close()
...
def _zapisz_opening_odds(liga: str, home: str, away: str,
                          oh: float, od: float, oa: float):
    """Zapisuje kursy otwarcia — tylko raz per mecz (pierwsze pobranie)."""
    try:
        init_db()
        con = sqlite3.connect(DB_FILE)
        con.execute(
            "INSERT OR IGNORE INTO odds_history"
            "(liga,home,away,fetched_at,odds_h,odds_d,odds_a,is_opening) VALUES(?,?,?,?,?,?,?,1)",
            (liga, home, away, datetime.now().isoformat(), oh, od, oa))
        con.commit()
        con.close()
    except Exception:
        pass
...
def _pobierz_trend_kursow(liga: str, home: str, away: str,
                           curr_h: float, curr_d: float, curr_a: float) -> dict:
    """
    Zwraca trend kursów vs kursy otwarcia (CLV proxy).
    dp > 0 = implied probability rośnie = smart money wchodzi = Market Confirmation.
    dp < 0 = market idzie przeciwko = Market Divergence.
    """
    try:
        init_db()
        con = sqlite3.connect(DB_FILE)
        row = con.execute(
            "SELECT odds_h, odds_d, odds_a FROM odds_history "
            "WHERE liga=? AND home=? AND away=? AND is_opening=1",
            (liga, home, away)).fetchone()
        con.close()
    except Exception:
        row = None

    if not row:
        _zapisz_opening_odds(liga, home, away, curr_h, curr_d, curr_a)
        return {"has_trend": False}

    open_h, open_d, open_a = row
    if not all([open_h, open_d, open_a, curr_h, curr_d, curr_a]):
        return {"has_trend": False}
    ...
    try:
        oh_p, od_p, oa_p = _impl(open_h, open_d, open_a)
        ch_p, cd_p, ca_p = _impl(curr_h, curr_d, curr_a)
    ...
    return {
        ...
        "curr_h": curr_h, "curr_d": curr_d, "curr_a": curr_a,
        # Zmiana implied probability w pp (dodatnia = smart money na tę opcję)
        "dph": round((ch_p - oh_p) * 100, 2),
        "dpd": round((cd_p - od_p) * 100, 2),
        "dpa": round((ca_p - oa_p) * 100, 2),
        # Zmiana kursu
        "dh": round(curr_h - open_h, 3),
        "dd": round(curr_d - open_d, 3),
        "da": round(curr_a - open_a, 3),
    }
...
def kelly_walkforward(liga: str, schedule: pd.DataFrame) -> dict:
    """
    Prawdziwy walk-forward Kelly bez look-ahead bias.
    Dla każdej kolejki K: trenuje model na danych K-1, predykuje K, weryfikuje wynik.
    Zwraca dict z bk_vals, kol_nums, roi, n, hr, dd lub {} jeśli za mało danych.
    """
    csv_code = LIGI[liga]["csv_code"]
    hist = load_historical(csv_code)
    if hist.empty:
        return {}

    hist_curr = hist[hist["_sezon"] == "biezacy"].copy() \
        if "_sezon" in hist.columns else hist.copy()
    if len(hist_curr) < 5:
        return {}

    if "Date" in hist_curr.columns:
        hist_curr = hist_curr.sort_values("Date").reset_index(drop=True)

    # Mapa (data, mapped_home, mapped_away) -> round
    sched_played = schedule[
        schedule["date"].apply(lambda d: pd.Timestamp(d).date() < __import__("datetime").date.today())
    ].copy() if not schedule.empty else pd.DataFrame()
    if sched_played.empty:
        return {}

    date_to_round = {}
    for _, sr in sched_played.iterrows():
        dt   = pd.Timestamp(sr["date"]).date()
        rnd  = int(sr["round"])
        sh   = map_nazwa(sr["home_team"])
        sa   = map_nazwa(sr["away_team"])
        date_to_round[(dt, sh, sa)] = rnd
        date_to_round[(dt, sa, sh)] = rnd

    # Przypisz kolejkę per mecz przez datę
    hist_curr["_round"] = None
    for idx, hr in hist_curr.iterrows():
        raw_h = str(hr.get("HomeTeam", ""))
        raw_a = str(hr.get("AwayTeam", ""))
        map_h = map_nazwa(raw_h)
        map_a = map_nazwa(raw_a)
        try:
            dt = pd.Timestamp(hr.get("Date")).date()
        except Exception:
            continue
        rv = (date_to_round.get((dt, map_h, map_a)) or
              date_to_round.get((dt, map_a, map_h)))
        if rv:
            hist_curr.at[idx, "_round"] = rv
        else:
            for d2 in [dt,
                       dt - __import__("datetime").timedelta(days=1),
                       dt + __import__("datetime").timedelta(days=1)]:
                for (dk, dh, da), rv2 in date_to_round.items():
                    if dk == d2 and (dh == map_h or da == map_a):
                        hist_curr.at[idx, "_round"] = rv2
                        break
                if hist_curr.at[idx, "_round"] is not None:
                    break

    hist_curr = hist_curr[hist_curr["_round"].notna()].copy()
    if hist_curr.empty:
        return {}
    hist_curr["_round"] = hist_curr["_round"].astype(int)
    rounds = sorted(hist_curr["_round"].unique())

    KS   = 1000.0
    KF   = 0.125 * KELLY_FRAC_SCALE
    KMAX = 0.05; KEV = 0.05; KMAX_ODDS = 3.50

    bk = KS; bk_flat = KS
    bk_vals = []; flat_vals = []; kol_nums = []
    n_typy = 0; n_traf = 0; kelly_pnl = 0.0
    peak = KS; max_dd = 0.0
    eq_rows = []

    for rnd in rounds:
        hist_before = hist_curr[hist_curr["_round"] < rnd].copy()
        if len(hist_before) < 10:
            continue
        try:
            sdf  = oblicz_wszystkie_statystyki(hist_before.to_json())
            slig = _oblicz_srednie_impl(hist_before.to_json())
        except Exception:
            continue
        if sdf.empty:
            continue

        forma = oblicz_forme(hist_before)
        rho_kf = float(slig.get("rho", -0.13))
        n_kf   = int(slig.get("n_biezacy", len(hist_before)))

        mecze_k = hist_curr[hist_curr["_round"] == rnd]
        for _, hr in mecze_k.iterrows():
            raw_h = str(hr.get("HomeTeam", ""))
            raw_a = str(hr.get("AwayTeam", ""))
            if not raw_h or not raw_a: continue
            if raw_h not in sdf.index or raw_a not in sdf.index: continue

            try:
                lh, la = oblicz_lambdy(raw_h, raw_a, sdf, slig, forma,
                                       csv_code=csv_code)[:2]
                pred = predykcja_meczu(lh, la, rho=rho_kf,
                                       csv_code=csv_code, n_train=n_kf)
            except Exception:
                continue

            typ = pred["typ"]; p = pred["p_typ"]; fo = pred["fo_typ"]

            try:
                hg = int(hr["FTHG"]); ag = int(hr["FTAG"])
            except Exception:
                continue

            w1x2 = "1" if hg > ag else ("2" if ag > hg else "X")
            traf = {"1": w1x2=="1","X": w1x2=="X","2": w1x2=="2",
                    "1X": w1x2 in ("1","X"),"X2": w1x2 in ("X","2")}.get(typ, False)

            def _gk(ch, cd, ca, row, t):
                try:
                    oh=float(row.get(ch) or 0)
                    od=float(row.get(cd) or 0)
                    oa=float(row.get(ca) or 0)
            ...
            k_buk = (_gk("MaxCH","MaxCD","MaxCA",hr,typ) or
                     _gk("PSCH","PSCD","PSCA",hr,typ) or
                     _gk("PSH","PSD","PSA",hr,typ) or
                     _gk("B365H","B365D","B365A",hr,typ) or fo)

            k_eff = k_buk if (k_buk and not __import__("math").isnan(k_buk)) else fo
            if not k_eff or k_eff <= 1.0: continue

            ev = p * k_eff - 1.0 if (1.35 <= k_eff <= KMAX_ODDS) else None

            # Flat
            k_flat = k_buk if (k_buk and not __import__("math").isnan(k_buk)) else fo
            bk_flat += float(k_flat) - 1 if traf else -1

            eq_rows.append({"rnd": rnd, "raw_h": raw_h, "raw_a": raw_a,
                             "typ": typ, "p": p, "fo": fo, "k_buk": k_buk,
                             "k_eff": k_eff, "ev": ev, "traf": int(traf)})

        # Kelly per kolejka (top 3 EV>=5%)
        rnd_rows = [r for r in eq_rows if r["rnd"] == rnd and
                    r["ev"] is not None and r["ev"] >= KEV]
        rnd_rows = sorted(rnd_rows, key=lambda x: -x["ev"])[:3]
        for r in rnd_rows:
            pt = 0.5 + (r["p"] - 0.5) * KELLY_PROB_SCALE
            kb = r["k_eff"]; b = kb - 1
            f  = min(max((pt*b-(1-pt))/b, 0.0) * KF, KMAX)
            st = round(bk * f, 2)
            if st > 0:
                bk += st*b if r["traf"] else -st
                kelly_pnl += st*b if r["traf"] else -st
                n_typy += 1; n_traf += int(r["traf"])
            bk = max(bk, 0.01)
            if bk > peak: peak = bk
            dd = (peak - bk) / peak * 100
            if dd > max_dd: max_dd = dd

        bk_vals.append(round(bk, 2))
        flat_vals.append(round(bk_flat, 2))
        kol_nums.append(rnd)
    ...
    return {
        "bk":       round(bk, 2),
        "bk_flat":  round(bk_flat, 2),
        "roi":      round((bk - KS) / KS * 100, 2),
        "roi_flat": round((bk_flat - KS) / KS * 100, 2),
        "n":        n_typy,
        "hr":       round(n_traf / n_typy, 3) if n_typy else 0,
        "dd":       round(max_dd, 2),
...
def oblicz_kontekst_meczu(h: str, a: str, historical: pd.DataFrame,
    ...
    if not historical.empty:
        def _team_stats(team):
            ...
            # Efficiency Index: SOT/Shots — snajperzy vs maszynowcy
            if stats.get("avg_shots") and stats.get("avg_sot") and stats["avg_shots"] > 0:
                stats["efficiency"] = round(stats["avg_sot"] / stats["avg_shots"], 3)
            else:
            ...
            if stats.get("efficiency") and stats["efficiency"] > 0.42:
                tags.append("🎯 Snajperzy")
            if not tags:
        ...
        def _build_narrative(ctx, h, a):
            ...
            # 9. Efficiency
            h_eff = ctx.get("h_stats", {}).get("efficiency")
            a_eff = ctx.get("a_stats", {}).get("efficiency")
            if h_eff and a_eff:
                if h_eff > 0.42 and a_eff < 0.30:
                    parts.append(f"{h} strzela precyzyjnie ({h_eff:.0%} SOT/strzał) — groźni nawet z mniejszej liczby okazji")
                elif a_eff > 0.42 and h_eff < 0.30:
                    parts.append(f"{a} to snajperzy ({a_eff:.0%} SOT/strzał vs {h_eff:.0%} {h}) — groźni z kontry")
            # 8. Volatility
...
with st.sidebar.expander("💰 Kursy bukmacherskie", expanded=not bool(_oa_key)):
    ...
    if _OA_OK and _oa_key:
        ...
        # Zapisz kursy otwarcia dla CLV (pierwsze pobranie = opening)
        if _OA_OK and _oa_key and _oa_cached:
            for (_h_ev, _a_ev), _ev_data in _oa_cached.items():
                try:
                    if _ev_data.get("odds_h") and _ev_data.get("odds_d") and _ev_data.get("odds_a"):
                        _zapisz_opening_odds(
                            wybrana_liga, _h_ev, _a_ev,
                            float(_ev_data["odds_h"]),
                            float(_ev_data["odds_d"]),
                            float(_ev_data["odds_a"]))
                except Exception:
                    pass
...
if not historical.empty:
    ...
    if not schedule.empty and not srednie_df.empty and _oa_cached:
        ...
        for _, _sm in _start_mecze.iterrows():
            ...
            try:
                ...
                _start_top.append({
                    ...
                    "odds_trend": _pobierz_trend_kursow(
                        wybrana_liga, _sh, _sa,
                        float(_so["odds_h"]) if _so else 0,
                        float(_so["odds_d"]) if _so else 0,
                        float(_so["odds_a"]) if _so else 0
                    ) if _so else {},
                })
        ...
        if _top_val:
            ...
            for _tvc, _tv in zip(_tv_cols, _top_val):
                ...
                # CLV / Market Movement badge
                _ot = _tv.get("odds_trend", {})
                _clv_badge = ""
                if _ot.get("has_trend"):
                    _tv_typ = _tv["typ"]
                    _dp_map = {"1": _ot.get("dph",0), "X": _ot.get("dpd",0), "2": _ot.get("dpa",0),
                               "1X": max(_ot.get("dph",0), _ot.get("dpd",0)),
                               "X2": max(_ot.get("dpd",0), _ot.get("dpa",0))}
                    _dp = _dp_map.get(_tv_typ, 0)
                    if _dp >= 2.0:
                        _clv_badge = ("<span class='stat-pill' "
                            "style='background:#1a3a1f;color:#00e676;border:1px solid #00e67633'>"
                            f"📈 +{_dp:.1f}pp Market Conf.</span>")
                    elif _dp <= -2.0:
                        _clv_badge = ("<span class='stat-pill' "
                            "style='background:#3b1a1a;color:#ff7043;border:1px solid #ff704333'>"
                            f"📉 {_dp:.1f}pp Divergence</span>")
                    elif abs(_dp) >= 0.8:
                        _arr = "\u2191" if _dp > 0 else "\u2193"
                        _clv_badge = (f"<span class='stat-pill' style='color:#888'>"
                            f"Mkt {_arr}{abs(_dp):.1f}pp</span>")
                # EV sygnalizacja
                if _tv['ev'] >= 0.15:
                    ...
                    for idx, (_, mecz) in enumerate(mecze_dnia.iterrows()):
                        ...
                        with kolumna:
                            ...
                            with st.expander("▾ szczegóły", expanded=False):
                                ...
                                _dt1, _dt2, _dt3, _dt4 = st.tabs(["📊 Analiza", "🔍 Kontekst", "⚔️ H2H", "📈 Statystyki"])
    ...
    with tab3:
        ...
        with st.expander("💰 Symulacja Kelly – Equity Curve (Walk-Forward)", expanded=False):
            st.caption("Prawdziwy walk-forward: predykcja kolejki K z danych K-1. Zero look-ahead bias.")

            _wf_key = f"wf_{wybrana_liga}"
            if _wf_key not in st.session_state:
                with st.spinner("Walk-forward Kelly..."):
                    _wf_res = kelly_walkforward(wybrana_liga, schedule)
                st.session_state[_wf_key] = _wf_res
            else:
                _wf_res = st.session_state[_wf_key]

            # Przycisk odswiezenia
            if st.button("🔄 Przelicz walk-forward", key="_wf_refresh"):
                with st.spinner("Walk-forward Kelly..."):
                    _wf_res = kelly_walkforward(wybrana_liga, schedule)
                st.session_state[_wf_key] = _wf_res

            if not _wf_res:
                st.info("Za mało danych (minimum 3 mecze z przypisanymi kolejkami).")
            else:
                # Zapisz do session_state dla Multi-Liga
                st.session_state[f"ml_bk_{wybrana_liga}"] = {
                    "bk":       _wf_res["bk"],
                    "bk_flat":  _wf_res["bk_flat"],
                    "roi":      _wf_res["roi"],
                    "roi_flat": _wf_res["roi_flat"],
                    "n":        _wf_res["n"],
                    "dd":       _wf_res["dd"],
                    "bk_vals":  _wf_res["bk_vals"],
                    "kol_nums": _wf_res["kol_nums"],
                }

                _KS = float(st.session_state.get("bankroll", 1000.0))
                _KF = 0.125 * KELLY_FRAC_SCALE
                _KMAX = 0.05; _KEV = 0.05

                _bk_vals  = _wf_res["bk_vals"]
                _kol_nums = _wf_res["kol_nums"]
                _bk       = _wf_res["bk"]
                _bk_flat  = _wf_res["bk_flat"]
                _roi_kelly = _wf_res["roi"]
                _roi_flat  = _wf_res["roi_flat"]
                _kelly_typy = _wf_res["n"]
                _kelly_hr   = f"{_wf_res['hr']:.0%}" if _wf_res["n"] else "brak"
                _max_dd_k   = _wf_res["dd"]
                _kelly_pnl  = _wf_res.get("pnl", 0)
                _kc = "#4CAF50" if _bk >= _KS else "#F44336"

                st.caption(f"Walk-forward: {_wf_res.get('n_mecze', len(_bk_vals)*10)} meczów · "
                           f"{len(_kol_nums)} kolejek · model trenuje na danych K-1")

                _ek1, _ek2, _ek3, _ek4 = st.columns(4)
                _ek1.metric("💰 Bankroll", f"{_bk:.0f} zł", f"{_roi_kelly:+.1f}%")
                _ek2.metric("📋 Typów Kelly", _kelly_typy,
                            f"Hit {_kelly_hr}", delta_color="off")
                _ek3.metric("📉 Max Drawdown", f"{_max_dd_k:.1f}%",
                            "wysoki" if _max_dd_k > 20 else "ok",
                            delta_color="inverse" if _max_dd_k > 20 else "normal")
                _ek4.metric("📊 Flat", f"{_bk_flat:.0f} zł", f"{_roi_flat:+.1f}%",
                            delta_color="normal" if _roi_flat >= 0 else "inverse")

                if len(_kol_nums) >= 2:
                    import altair as _alt
                    _cdf = pd.DataFrame({"Kolejka": _kol_nums[:len(_bk_vals)],
                                         "Kelly (zł)": _bk_vals})
                    _ln  = _alt.Chart(_cdf).mark_line(color=_kc, strokeWidth=2).encode(
                        x=_alt.X("Kolejka:Q"), y=_alt.Y("Kelly (zł):Q", title="Bankroll (zł)"))
                    _rl  = _alt.Chart(pd.DataFrame({"y": [_KS]})).mark_rule(
                        color="#555", strokeDash=[4,4]).encode(y="y:Q")
                    st.altair_chart((_ln + _rl).properties(height=180),
                                    use_container_width=True)

                if _kelly_typy > 0:
                    st.caption(
                        f"{_kelly_typy} typów z EV≥5% · PnL: {_kelly_pnl:+.0f} zł · "
                        f"Flat: {_roi_flat:+.1f}%")
    ...
    with tab4:
        ...
        if _g_df.empty:
            ...
            for _ml_i, _ml_liga in enumerate(_ml_to_compute):
                if _ml_prog:
                    _ml_prog.progress(_ml_i / max(len(_ml_to_compute), 1),
                                      text=f"Walk-forward Kelly — {_ml_liga}...")
                _wf_key_ml = f"wf_{_ml_liga}"
                if _wf_key_ml in st.session_state and st.session_state[_wf_key_ml]:
                    _wf_ml = st.session_state[_wf_key_ml]
                else:
                    _wf_ml = kelly_walkforward(_ml_liga, schedule)
                    st.session_state[_wf_key_ml] = _wf_ml
                if not _wf_ml:
                    continue
                _ml_results[_ml_liga] = {
                    "bk":       _wf_ml["bk"],
                    "bk_flat":  _wf_ml["bk_flat"],
                    "roi":      _wf_ml["roi"],
                    "roi_flat": _wf_ml["roi_flat"],
                    "dd":       _wf_ml["dd"],
                    "n":        _wf_ml["n"],
                    "hr":       _wf_ml.get("hr", 0),
                    "bk_vals":  _wf_ml["bk_vals"],
                    "flat_vals": _wf_ml.get("flat_vals", []),
                    "kol_nums": _wf_ml["kol_nums"],
                }
