...
LEAGUES = {
    "Premier League": {
        ...
        "schedule_file": "terminarz_premier_2025.csv"
    },
    "La Liga": {
        "name": "La Liga",
        "country": "Hiszpania",
        ...
        "schedule_file": "terminarz_la_liga_2025.csv"
    },
    "Bundesliga": {
        "name": "Bundesliga",
        "country": "Niemcy",
        ...
        "schedule_file": "terminarz_bundesliga_2025.csv"
    },
    "Serie A": {
        "name": "Serie A",
        "country": "Włochy",
        ...
        "schedule_file": "terminarz_serie_a_2025.csv"
    },
    "Ligue 1": {
        "name": "Ligue 1",
        "country": "Francja",
        ...
        "schedule_file": "terminarz_ligue_1_2025.csv"
    }
}
...
def load_historical(data_url, league_code):
    """Pobiera dane historyczne dla wybranej ligi z football-data.co.uk"""
    try:
        with st.spinner(f'Pobieranie danych historycznych dla {league_code}...'):
...
# ==========================================
# WCZYTYWANIE TERMINARZA Z PLIKU
# ==========================================
...
def load_schedule_from_file(filename):
    ...
    try:
        if not os.path.exists(filename):
            st.warning(f"Plik {filename} nie istnieje")
            return pd.DataFrame()
        ...
        if not all(col in df.columns for col in required_cols):
            st.warning(f"Plik {filename} nie zawiera wymaganych kolumn. Oczekiwano: date, home_team, away_team")
            return pd.DataFrame()
        ...
        st.warning(f"Błąd wczytywania pliku {filename}: {e}")
        return pd.DataFrame()
...
with st.sidebar:
    ...
    st.caption(f"📋 Plik terminarza: {league_config['schedule_file']}")
...
historical = load_historical(league_config['data_url'], league_config['code'])

if historical.empty:
    st.error("Nie udało się pobrać danych historycznych. Spróbuj później.")
    st.stop()

# Wczytaj terminarz z pliku
schedule = load_schedule_from_file(league_config['schedule_file'])

if schedule.empty:
    st.warning(f"Nie udało się wczytać terminarza z pliku {league_config['schedule_file']}. Predykcje nie będą dostępne.")
...
with tab1:
    ...
    with c1:
        linia_gole = st.selectbox("Linia goli", [1.5, 2.5, 3.5, 4.5], index=1, key="gole")
        typ_gole = st.selectbox("Typ goli", ["Over", "Under"], key="typ_gole")
    with c2:
        linia_rogi = st.selectbox("Linia rożnych", [7.5, 8.5, 9.5, 10.5, 11.5, 12.5], index=2, key="rogi")
        typ_rogi = st.selectbox("Typ rożnych", ["Over", "Under"], key="typ_rogi")
    with c3:
        linia_kartki = st.selectbox("Linia kartek", [2.5, 3.5, 4.5, 5.5], index=1, key="kartki")
        typ_kartki = st.selectbox("Typ kartek", ["Over", "Under"], key="typ_kartki")
    ...
    if not schedule.empty:
        ...
        st.warning("Brak danych terminarza. Załaduj plik CSV, aby zobaczyć predykcje.")
...
with tab3:
    ...
    st.write("Dane uwzględniają atut własnego boiska oraz wagę ostatnich meczów (ostatnie 10 meczów).")
    ...
    with col_info1:
        st.metric("📊 Liczba meczów w bazie", len(historical))
    with col_info2:
        st.metric("🔄 Ostatnia aktualizacja", historical['Date'].max().strftime('%d.%m.%Y') if not historical.empty else '—')
    with col_info3:
        st.metric("📋 Drużyn w lidze", len(srednie_df))
    
    st.caption(f"Źródło danych historycznych: football-data.co.uk")
    st.caption(f"Źródło terminarza: plik {league_config['schedule_file']}")
